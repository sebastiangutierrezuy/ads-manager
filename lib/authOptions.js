...
// IMPORTANTE: no hacemos throw acá. Next.js evalúa este módulo durante el
// build, y si las env vars todavía no están disponibles (típico en CI), el
// build falla aunque la config en producción esté bien. Si SECRET falta en
// runtime, NextAuth tira un error claro al primer request.
if (!SECRET) {
  console.warn(
    '[auth] AUTH_SECRET (o NEXTAUTH_SECRET) no está seteado. ' +
    'Si esto es build, asegurate de configurarlo en producción. ' +
    'Local: openssl rand -base64 32 → .env.local'
  );
} else {
  console.log(
